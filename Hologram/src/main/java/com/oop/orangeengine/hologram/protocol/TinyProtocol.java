...
public abstract class TinyProtocol {
	...
	private static final Class minecraftServerClass = findClass("{nms}.MinecraftServer");
	private static final Class serverConnectionClass = findClass("{nms}.ServerConnection");
	private static final Field getMinecraftServer = OSimpleReflection.getField(findClass("{obc}.CraftServer"), minecraftServerClass);
	private static final Field getServerConnection = OSimpleReflection.getField(minecraftServerClass, serverConnectionClass);
	private static final Method getNetworkMarkers = OSimpleReflection.getMethod(serverConnectionClass, null, List.class, serverConnectionClass);

	// Packets we have to intercept
	private static final Class<?> PACKET_LOGIN_IN_START = OSimpleReflection.findClass("{nms}.PacketLoginInStart");
	private static final Field getGameProfile = OSimpleReflection.getField(PACKET_LOGIN_IN_START, GameProfile.class);
	...
	private void registerChannelHandler() {
		try {
			...
			for (int i = 0; looking; i++) {
				List<Object> list = (List<Object>) OSimpleReflection.getField(serverConnection.getClass(), null, List.class, i).get(serverConnection);
	...
	public Channel getChannel(Player player) {
		...
		try {
			...
			throw new IllegalStateException("Failed to get channel for " + player.getName(), thrw);
		}
	...
	private final class PacketInterceptor extends ChannelDuplexHandler {
		...
		private void handleLoginStart(Channel channel, Object packet) {
			if (PACKET_LOGIN_IN_START.isInstance(packet)) {
				...
				try {
					profile = (GameProfile) getGameProfile.get(packet);
