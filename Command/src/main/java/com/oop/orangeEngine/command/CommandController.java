...
public class CommandController {
        ...
        commandMap.register(plugin.getName(), new Command(command.getLabel(), command.getDescription(), "none", new ArrayList<>(command.getAliases())) {
            ...
            public boolean execute(CommandSender sender, String cmdName, String[] args) {
                ...
                if (args.length == 0)
                    ...
                    OOPCommand subCommand = command.getSubCommands().get(secondArg);
                    if (subCommand != null)
    ...
    public List<String> handleTabComplete(OOPCommand command, String[] args) {
    ...
    private void handleCommand(String[] args, CommandSender sender, OOPCommand command) {
        ...
        for (RequirementMapper requirementMapper : command.getRequirementSet()) {
            ...
            Pair<Boolean, String> mapped = requirementMapper.accepts(sender);
            if (!mapped.getKey()) {
    ...
    private void handleProperUsage(OOPCommand command, CommandSender sender) {
        ...
        if (command.getSubCommands().isEmpty()) {
            ...
            //Advanced
            OMessage message = new OMessage();
            message.appendLine(colorScheme.getSecondColor() + "&l---==== " + colorScheme.getMainColor() + " " + command.getLabel() + " Help");
