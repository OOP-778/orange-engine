...
public enum OSound {
    ...
    OSound(String... versionDependentNames) {
    ...
    public static final Sound fallback() {
        return Sound.valueOf(OVersion.isAfter(9) ? "ENTITY_PLAYER_LEVELUP" : "LEVEL_UP");
    }
    ...
    public static final Sound convert(String soundName) {
        ...
        for (final OSound oSound : values())
            for (final String name : oSound.versionDependentNames)
                if (name.equalsIgnoreCase(soundName))
                    sound = oSound;

        return sound != null ? sound.sound() : fallback();
    }
