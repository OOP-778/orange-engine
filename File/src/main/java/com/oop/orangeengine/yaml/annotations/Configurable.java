...
public interface Configurable {
            ...
            for (Field field : getClass().getDeclaredFields()) {
                ...
                for (Annotation annotation : field.getDeclaredAnnotations()) {
                    ...
                    if (annotation instanceof ConfigValue) {
                        ...
                        if (!section.isPresentValue(configValue.path()))
                            throw new IllegalStateException("Failed to find configuration getValue at (" + configValue.path() + ") at file (" + section.getConfiguration().getOFile().getFile().getName() + ")");

                        reqProductType = field.getType();
                        Object value = section.getValue(configValue.path()).getValue();

                        //Check if configuration getValue is parsable to field type
                        if (value.getClass().isInstance(reqProductType))
                            throw new IllegalStateException("Failed to parse configuration getValue at (" + configValue.path() + ") at file (" + section.getConfiguration().getOFile().getFile().getName() + ") as " + reqProductType.getTypeName());
                        ...
                        if (!section.isPresentSection(sectionValue.path()))
                            throw new IllegalStateException("Failed to find configuration getSection at (" + sectionValue.path() + ") at file (" + section.getConfiguration().getOFile().getFile().getName() + ")");
                        ...
                        if (field.getGenericType() instanceof ParameterizedType) {
                            ...
                            ConfigurationSection section2 = section.getSection(sectionValue.path());

                            List list = (List) field.get(this);

                            for (ConfigurationSection childSection : section2.getSections().values()) {
                            ...
                            Object mappedObject = SectionMappers.map(section2, reqProductType);
    ...
    default void load(OConfiguration configuration) {
        try {
